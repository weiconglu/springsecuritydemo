...
//		jdbcTokenRepositoryImpl.setCreateTableOnStartup(true); // <------ 这里设置为false，不让spring security自动建表，将建表操作配置到application.properties中
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		// 在登录成功界面加上个登出
		http.logout().logoutUrl("/logout") // <------ 这里是个访问路径，前端页面提交的地址是<a href="/springbootaction/logout">退出</a>
		...
		http.formLogin() // <------自定义登录页面设置
				...
				/*
				 * 验证传回的用户名密码的请求路径，由spring security自己生成， 与verify.html 页面相对登录验证提交的地址相对应-> <form
				 * action="/springbootaction/login_verify" method="post">
				...
				.antMatchers("/login_verify", "/hello", "/swagger-resources/**", "/webjars/**", "/v2/**",
						"/api-docs/**", "/swagger-ui.html/**")
				.permitAll().antMatchers("/books/**").hasAuthority("root") // <------有root角色权限的才可以访问
...
//				.antMatchers("").hasAnyRole("guest")
				.anyRequest().authenticated().and().rememberMe() // <------ 开启记住我功能，在verify.html中加入<input
																	// type="checkbox"
																	// name="remember-me">保持登录注意其name必须为remember-me
				...
				.userDetailsService(userDetailsService) // <------
														// 操作用户表的com.example.demo.service.impl.UserDetailsServiceImpl实例
